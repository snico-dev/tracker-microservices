...
//FcsTab unicod
var FcsTab = []int{0x0000, 0x1189, 0x2312, 0x329b, 0x4624,
...
//GetFcs return fcs
func GetFcs(fcs int, src int) int {
	xor := 0
	iresult := 0
	xor = fcs
	xor ^= src
	str := FcsTab[xor&0x00ff]
	iresult = (((fcs) >> 8) ^ str)
	return iresult
}

//Make return encripty data
func Make(psrc []byte, ilen int) (string, error) {
	crc := 0
	crc = FcsStart

	for i := 0; i < ilen; i++ {
		crc = GetFcs(crc, int(psrc[i]))
	}

	crc ^= FcsStart
	crcCalc := crc & 0xffff

	bytes := convert.FromUnit16ToByte(uint16(crcCalc))
	return convert.FromByteToHex(bytes)
}

//GetDescCrcBytes convert crc to bytes
func GetDescCrcBytes(asc string) ([]byte, error) {
	desc := asc[len(asc)-2:len(asc)] + asc[0:len(asc)-2]
	return hex.DecodeString(desc)
}

//GetDescCrcHex convert crc to hex
func GetDescCrcHex(asc string) string {
	desc := asc[len(asc)-2:len(asc)] + asc[0:len(asc)-2]
	return desc
}
